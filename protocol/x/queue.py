...
class RequestQueue:
    ...
    def _wait_for_rate_limit(self):
        ...
        with self.rate_limit_lock:
            ...
            if time_since_last_request < required_gap:
                ...
                logger.debug(f"Rate limiting: sleeping for {
    ...
    def _handle_request(self, request_type: str, request_data: Dict[str, Any]):
        ...
        with self.lock:
            ...
            logger.debug(f"Active requests increased to {
        ...
        try:
            ...
            with self.lock:
                self.active_requests -= 1
                logger.debug(f"Active requests decreased to {
                             self.active_requests}")
    ...
    def _retry_request(self, request_type: str, request_data: Dict[str, Any], retries: int = DEFAULT_RETRIES):
        ...
        for attempt in range(retries):
            try:
                logger.warning(f"Retrying {request_type} request: {
