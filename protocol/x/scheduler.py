...
class XSearchScheduler:
    ...
    def _prepare_search_query(self, term: Dict[str, Any]) -> Dict[str, Any]:
        ...
        a 2-day lookback period and the configured result count. The time range uses
        2 days ago to ensure complete data collection.
        ...
        # Format the query string with 2 days ago date
        query = f"({term['query']}) since:{two_days_ago.strftime('%Y-%m-%d')}"
