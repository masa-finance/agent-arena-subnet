...
VALIDATOR_DB_PATH = f"{VALIDATOR_DB_BASE}/search/terms"
...
SCHEDULER_INTERVAL = int(os.getenv('SCHEDULER_INTERVAL_MINUTES', DEFAULT_INTERVAL_MINUTES))
...
class XSearchScheduler:
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized XSearchScheduler with interval={interval_minutes}min, "
    ...
    def _get_search_terms(self) -> List[Dict[str, Any]]:
        ...
        """
        try:
            api_url = f"{VALIDATOR_DB_BASE_URL.rstrip('/')}/{VALIDATOR_DB_PATH.lstrip('/')}"
