...
from protocol.validator.posts_scorer import AgentScorer
...
def test_agent_scoring_with_real_data():
    ...
    if len(posts) > 0:
        ...
        if tweets:
            ...
            print(
                f"Timestamp present: {tweet.get('Timestamp') is not None}",
                file=sys.stderr,
            )
            print(f"Likes present: {tweet.get('Likes') is not None}", file=sys.stderr)
            print(
                f"Retweets present: {tweet.get('Retweets') is not None}",
                file=sys.stderr,
            )
            print(
                f"Replies present: {tweet.get('Replies') is not None}", file=sys.stderr
            )
