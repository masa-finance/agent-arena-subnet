...
def test_request_queue_live():
    ...
    # Initialize queue
    rq = RequestQueue(max_concurrent_requests=2)
    logger.debug("Created RequestQueue with max_concurrent_requests=2")
    ...
    for req_type, data, priority in test_requests:
        logger.info(f"Adding request: type={req_type}, data={data}, priority={priority}")
...
def test_request_queue_priorities():
    ...
    rq = RequestQueue(max_concurrent_requests=1)  # Single thread for predictable ordering
    logger.debug("Created RequestQueue with max_concurrent_requests=1")
...
def test_request_queue_concurrent():
    ...
    rq = RequestQueue(max_concurrent_requests=3)
    logger.debug("Created RequestQueue with max_concurrent_requests=3")
